...
<button role="button" class="btn-crud btn-crud-warning btn-crud-sm" data-bs-toggle="modal"
    data-bs-target=".formEdit{{ $kelas->id }}">
    <i class="fas fa-edit"></i> <span class="d-none d-sm-inline">Edit</span>
</button>

<!-- Modal -->
<div class="modal fade formEdit{{ $kelas->id }} modal-crud" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.kelas.update', $kelas->id) }}" class="form-crud">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit mr-2"></i>Edit Kelas</h5>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="matkul_dibuka_id" class="form-select @error('matkul_dibuka_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($matkuls as $matkul)
                                        <option value="{{ $matkul->id }}" {{ old('matkul_dibuka_id', $kelas->matkul_dibuka_id) == $matkul->id ? 'selected' : '' }}>{{ $matkul->nama_matkul }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tahun_ajaran_id" class="form-select @error('tahun_ajaran_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tahunAjarans as $tahunAjaran)
                                        <option value="{{ $tahunAjaran->id }}" {{ old('tahun_ajaran_id', $kelas->tahun_ajaran_id) == $tahunAjaran->id ? 'selected' : '' }}>{{ $tahunAjaran->tahun_ajaran }}</option>
                                    @endforeach
                                </select>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    var modalEl = document.querySelector('.formEdit{{ $kelas->id }}');
